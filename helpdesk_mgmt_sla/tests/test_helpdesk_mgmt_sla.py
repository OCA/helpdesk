...
class TestHelpdeskMgmtSla(CommonHelpdeskMgmtSla):
    ...
    def test_sla_write_permission(self):
        ...
        """
        ticket1 = self.get_ticket(self.team2, {"user_id": self.user_sla.id})
        self.assertFalse(
            self.env["helpdesk.ticket.sla"]
            .with_user(self.user_sla)
            .search([("ticket_id", "=", ticket1.id)])
            .check_access_rule("write")
        )
    ...
    def test_sla_no_write_permission(self):
        ...
        self.user_sla.groups_id = [
            Command.unlink(self.env.ref("helpdesk_mgmt.group_helpdesk_user_own").id)
        ]
        with self.assertRaises(AccessError):
            ...
            sla.check_access_rights("write")
            sla.check_access_rule("write")
