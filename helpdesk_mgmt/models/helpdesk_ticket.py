...
class HelpdeskTicket(models.Model):
    ...
    def _get_default_stage_id(self):
        return self.env['helpdesk_mgmt.ticket.stage'].search([], limit=1).id
    ...
    def _read_group_stage_ids(self, stages, domain, order):
        stage_ids = self.env['helpdesk_mgmt.ticket.stage'].search([])
        return stage_ids

    stage_id = fields.Many2one(
        'helpdesk_mgmt.ticket.stage',
        string='Stage',
    ...
    tag_ids = fields.Many2many('helpdesk_mgmt.ticket.tag')
    company_id = fields.Many2one(
        'res.company',
        string="Company",
        default=lambda self: self.env['res.company']._company_default_get(
            'helpdesk_mgmt.ticket')
    )
    channel_id = fields.Many2one(
        'helpdesk_mgmt.ticket.channel',
        string='Channel',
        help='Channel indicates where the source of a ticket'
             'comes from (it could be a phone call, an email...)',
    )
    category_id = fields.Many2one('helpdesk_mgmt.ticket.category',
                                  string='Category')
    team_id = fields.Many2one('helpdesk_mgmt.ticket.team')
    ...
    def write(self, vals):
        for ticket in self:
            ...
            if vals.get('stage_id'):
                stage_obj = self.env['helpdesk_mgmt.ticket.stage'].browse(
                    [vals['stage_id']])
