...
class CustomerPortal(CustomerPortal):
    ...
    def _prepare_portal_layout_values(self):
        ...
        ticket_count = request.env['helpdesk_mgmt.ticket'].search_count(
            [('partner_id', 'child_of', partner.id)])
        values['ticket_count'] = ticket_count
        return values

    def _helpdesk_ticket_check_access(self, ticket_id):
        ticket = request.env['helpdesk_mgmt.ticket'].browse([ticket_id])
    ...
    def portal_my_tickets(
        ...
        HelpdesTicket = request.env['helpdesk_mgmt.ticket']
        partner = request.env.user.partner_id
        ...
        for stage in request.env['helpdesk_mgmt.ticket.stage'].search([]):
    ...
    def _ticket_get_page_view_values(self, ticket, **kwargs):
        closed_stages = request.env['helpdesk_mgmt.ticket.stage'].search(
            [('closed', '=', True)])
        values = {
            'page_name': 'ticket',
            'ticket': ticket,
