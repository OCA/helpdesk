...
class TestHelpdeskTicketSubcategory(test_helpdesk_ticket.TestHelpdeskTicket):
    ...
    def setUpClass(cls):
        ...
        cls.category1 = Category.create({'name': "Cat1", 'parent_id': cls.root_category.id})
        cls.category2 = Category.create({'name': "Cat2", 'parent_id': cls.root_category.id})
        cls.ticket.write(
    ...
    def test_helpdesk_category(self):
        self.assertEqueal(
            self.category1.code,
            'root-cat1',
            "Helpdesk Category: Wrong category code"
        )
